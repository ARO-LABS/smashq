...
### Geändert
- **Einstellungen aufgeräumt** (#52): Tabs von 7 auf 5 konsolidiert — „Terminal" ist jetzt die Sektion „Terminal-Verlauf" im Sessions-Tab, „Erweitert" (Debug-Logging) eine Sektion im System-Tab; kein Feld und kein Text entfällt, gespeicherte Werte bleiben unverändert. Einheitliche Schalter in App-Akzentfarbe (statt dreier Checkbox-Varianten, teils im OS-Blau), einheitliche Auswahlfelder mit weichen Ecken, durchgängige Tastatur-Fokus-Ringe (2px Akzent-Outline) und aktiver Navigations-Eintrag mit Akzent-Kante links (Design-System-Pattern).
