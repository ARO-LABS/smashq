...
### 2026-07-21 — Roundtrip-Wächter für partialize: Live-State zurückdrehen reicht nicht, weil persist auch `api.setState` patcht

**Kontext:** Review-Nacharbeit PR #46 (#21): Die Roundtrip-Tests (`setzen → rehydrate → Wert noch da`) diskriminierten eine fehlende partialize-Zeile nicht, weil zustands `merge` den LIVE-State als current nutzt — steht der Wert schon im Speicher, bleibt der Test grün, auch wenn der Blob das Feld nie enthielt. Der vorgeschlagene Fix (nach dem Setter den Live-State via `setState` zurückdrehen, dann `rehydrate()`) klang schlüssig, war aber empirisch rot TROTZ vorhandener partialize-Zeile.

**Fehler → Korrektur:** Übersehen, dass die persist-Middleware nicht nur das an den Creator gereichte `set`, sondern auch `api.setState` patcht (zustand v4.5 `middleware.js`: `savedSetState(state, replace); void setItem();`) — das „Zurückdrehen" schreibt den partialized Live-State also sofort selbst in den Blob und zerstört genau den Blob-Zustand, den der Test prüfen wollte. Korrektur: Blob nach dem Setter-Write EINFANGEN (`localStorage.getItem`), Live-State zurückdrehen (überschreibt den Blob — egal), eingefangenen Blob RE-SEEDEN, dann `rehydrate()`. Beide Wächter (autoUpdateEnabled, defaultPermissionMode) per Red-Green bewiesen: partialize-Zeile temporär raus → rot, wieder rein → grün.

**Regel:** (1) Ein Persistenz-Wächtertest ist erst fertig, wenn die verhinderte Regression real eingebaut und der Test dabei ROT gesehen wurde — „müsste diskriminieren" zählt nicht. (2) Bei zustand-persist gilt: JEDER `setState`-Aufruf (auch `useStore.setState` von außen) triggert einen Persist-Write des partialized Live-States; Tests, die Blob und Live-State auseinanderziehen wollen, müssen den Blob capturen und nach der State-Manipulation re-seeden. (3) Auch präzise Review-/Task-Vorgaben für Test-Mechanik empirisch verifizieren, bevor man sie als korrekt übernimmt — hier hätte die Vorgabe 1:1 umgesetzt einen dauerhaft roten Test ergeben.
