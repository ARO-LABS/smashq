...
### 2026-07-03 — macOS-„Update-Check fehlgeschlagen"-Toast beim Start: Updater sucht Plattform VOR dem Versionsgate + isBenign-Regex traf die falsche Wortwahl

**Kontext:** Mac-User meldete Fehler-Toast „Update fehlgeschlagen" ~15 s nach App-Start (der reale macOS-`.app`-Build aus Runde 2). Systematic-Debugging, Root-Cause durch die `tauri-plugin-updater`-2.10.1-Quelle im Cargo-Registry getracet statt geraten.

**Fehler:** Zwei zusammenwirkende Ursachen. (1) Die Release-Pipeline publiziert nur Windows-Targets in `latest.json` (`windows-x86_64`, `windows-x86_64-nsis`) — kein `darwin-aarch64`. (2) `tauri-plugin-updater` ruft in `check()` `get_urls()` **UNBEDINGT auf, VOR** dem `should_update`-Versionsgate (`updater.rs`: Zeile 536 `get_urls(...)?` steht vor Zeile 538 `if should_update`). Auf macOS findet `get_urls` seinen Ziel-Target nicht → `Err(TargetsNotFound(["darwin-aarch64"]))` — und zwar **immer**, selbst bei identischer Version (1.0.21 == 1.0.21). Meine erste Annahme „gleiche Version → keine Plattform-Suche → kein Fehler" war falsch (Reihenfolge im Plugin ist umgekehrt). Der `isBenign`-Filter in `useAutoUpdate.ts` fing es nicht: er matcht `not found`, aber die `TargetsNotFound`-Message lautet „None of the fallback platforms `[...]` **were found** in the response `platforms` object" — „were found", nicht „not found" → als echter Fehler klassifiziert → `status:"error"` → Toast. (Die Einzel-Target-Variante `TargetNotFound` sagt „**was not found**" und wäre über `not found` schon benigne gewesen — nur die Vec-Variante schlüpfte durch.)

**Korrektur:** `none of the fallback platforms` in den `isBenign`-Regex aufgenommen — „keine Artefakte für diese Plattform" heißt schlicht „kein Update für mich", kein Alarm-wert (semantisch dieselbe Klasse wie die schon vorhandenen network/404/dns-Fälle). TDD: Repro-Test mit dem exakten Plugin-Fehler-String zuerst RED (`expected 'error' to be 'idle'`), nach dem Fix GREEN; Guard-Test stellt sicher, dass echte Nicht-benigne Fehler (z. B. Signaturfehler) weiterhin toasten. Windows unberührt (findet `windows-x86_64-nsis`). Geschützter Pfad → Triple-Check: 199/199 Updater-Pfad-Tests grün, Prod-`.app` gebaut (Fix-String im Bundle nachgewiesen), visuelle macOS-Smoke an den User delegiert (screencapture per TCC blockiert).

**Regel:** (1) Der Tauri-Updater sucht die Plattform-URL VOR dem Versionsvergleich — jede Plattform ohne `latest.json`-Artefakt wirft in `check()` *immer*, unabhängig von der Version. Bei Multi-Plattform-Distribution mit nur teilweise gepflegten Targets ist das der Default-Fall, nicht die Ausnahme. (2) Fehler-Klassifizierungs-Regexe gegen die EXAKTEN `#[error(...)]`-Strings der installierten Lib-Version prüfen (`error.rs` im Cargo-Registry lesen), nicht gegen die vermutete Formulierung — „not found" vs „were found" ist hier der Unterschied zwischen still-schlucken und Alarm-Toast. (3) „App startet auf Plattform X mit Fehler" bei einem geschützten Pfad: erst durch die Lib-Quelle tracen (wo genau entsteht der Err?), dann minimal + test-getrieben fixen. Verwandt: [[2026-07-03-macos-session-start]] (dieselbe Windows-first-Annahmen-Klasse, eine Ebene höher).
