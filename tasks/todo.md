...
- **Whole-Tile-Drag — implementiert (`fix/whole-tile-drag`, 2026-07-02):** Session-Kacheln, Favoriten-Karten und Favoriten-Gruppen sind per Klick-und-Ziehen auf der ganzen Kachel verschiebbar; Grip-Handles komplett entfernt (inkl. verwaistem dragHandle-Icon). Mechanik: dnd-kit-Listener auf dem Sortable-Root (Gruppen: Header-Zeile wegen Bubbling), neuer geteilter `SmartPointerSensor` (ignoriert button/a/input/textarea/select/[data-no-dnd], behält isPrimary+button===0-Guards), 6px-Aktivierungsdistanz trennt Klick von Drag. Gates grün: tsc, 132 Testdateien vitest, eslint, npm build. **Offen:** visuelle Smoke durch User (Drag überall, Klick/Doppelklick/Rechtsklick/Buttons/Chevron/Rename-Input), dann Merge.
- **Logging-Hardening — implementiert (`fix/logging-hardening`, 2026-07-02):** 34-Agenten-Armada-Review des NDJSON-Logging-Features → 24 verifizierte Findings (3 High). Gefixt in 9 Commits: logError(undefined)-Crash, Persistenz-Gate vom Master-Gate entkoppelt + Pre-Wiring-Buffer, Rust-Write-Fehler propagiert (append_frontend_logs kein Silent-Ok mehr) + Input-Caps, Read-Pfad UTF-8-robust/rotations-übergreifend/async, Close-Flush-in-flight-Race, Listener-Races (cancelled/disposed-Flags), Cross-Window-Log-Sync (Broadcast + Snapshot — Split-Brain-High), sanitizePreferences in beiden Hydration-Hooks, Dead Code (parseBackendLogLine, open_log_window). Gates grün: tsc, 2373 vitest, npm build, cargo fmt/clippy/test (514). **Offen:** manuelle Smoke (Protokolle-Fenster sieht Hauptfenster-Fehler live + beim Mount) durch User, dann Merge.
- **Kanban-Delta-Armada-Review + Fixes — erledigt (`fix/armada-kanban-delta` → `master`, 2026-06-14):** 34-Agenten-Review des ungeprueften Deltas seit `bc5c3e3` (Kanban-Single-Board, Org-Board-Backend, adpError). 22 verifizierte Findings, keine echten Laufzeit-/Security-Bugs. Gefixt (3 Commits): gh-Fehler ueber `run_gh` vereinheitlicht, TOCTOU `gh_missing`, Wrong-Type-Node via `__typename`, `validate_id` ASCII, `ensure_gh`-Reihenfolge, Double-Fetch behoben, Toast-Close Token/A11y, totes `"kanban"`-ConfigSubTab, `projectStore` `partialize`, redundanter Cast, + Tests (reaktiver Store-Fixture, Owner-Switch-Race, adpError-Coverage). Gates gruen: cargo test/clippy/fmt, tsc, eslint, 2350 unit + 70 integration, `npm build`.
