...
- **Live-Log-Bugfixes — komplett** (`fix/log-write-bugs` → `master` `82b68c3`, 2026-06-07): Aus den Logs der installierten .exe gefundene Fehler behoben. (1) `atomic_write`-Race: fester Temp-Name `path.with_extension("tmp")` kollidierte, sobald zwei Webviews dieselbe Datei schrieben (Cross-Window-Sync ließ Haupt- + Aufgaben-Fenster `tasks.json` gleichzeitig persistieren) → `FILE_IO_ERROR ... os error 2`. Fix: unique Temp `tmp.{pid}.{counter}` (härtet alle Caller). (2) Redundanter Empfänger-Persist: `applyRemoteTasks`-`setState` triggerte zustand-persist im Empfänger → Doppel-Write + doppelte Backup-Rotation. Fix: synchrones `suppressPersist`-Flag um das `setState`. (3) Diff-Fenster-ACL: Label `diff-{id}` matchte keine Capability-Glob → `event:listen` verweigert. Fix: `"diff-*"` in `capabilities/default.json`. (4) Log-Viewer-UX: neueste Einträge oben + Zeit `HH:MM:SS` (ohne ms). Via Workflow (3 TDD-Implementer + adversariale Review) + 2 TDD-Commits. Gates grün: tsc/eslint, 2311 vitest, Rust fmt/clippy/431 cargo-test. **Offen:** `.exe`-Smoke durch User (Task mit zwei Fenstern löschen → kein FILE_IO_ERROR; Diff-Fenster → kein ACL-Fehler; Log-Viewer neueste-oben + HH:MM:SS). Kein geschützter Updater-Pfad berührt.
